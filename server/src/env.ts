...
const envSchema = z.object({
  ...
  JWT_SECRET: z
    .string()
    .min(8, 'JWT_SECRET deve ter ao menos 8 caracteres')
    .refine(
      (secret) => !FORBIDDEN_JWT_SECRETS.includes(secret.toLowerCase()),
      'JWT_SECRET não pode ser um valor de exemplo ou inseguro'
    ),
  ADMIN_EMAIL: z.string().email().optional().default('admin@agencia.com'),
  ADMIN_PASSWORD: z.string().min(8).optional().default('admin123456'),
